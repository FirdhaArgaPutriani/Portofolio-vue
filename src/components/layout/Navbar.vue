<template>
    <header>
        <div class="flex justify-between items-center p-8 lg:px-12 relative z-20">
            <div class="text-3x1 font-bold dark:text-white">LOGO</div>

            <!-- Mobile Toggle Button-->
            <div class="md:hidden z-30">
                <button class="block focus:outline-none" @click="isMenuOpen = !isMenuOpen">
                    <span v-if="isMenuOpen" class="text-5x1 md:text-primary text-white dark:text-white">
                        <Icon icon="material-symbols:close-rounded"/>
                    </span>
                    <span v-else class="text-5x1 md:text-primary text-white dark:text-white">
            ...
            >
                <ul class="flex flex-col items-center space-y-5 md:flex-row md:space-x-5 md:space-y-0">
                    <li v-for="item in Menu" :key="item.name">
                        ...
                        hover:text-secondary dark:text-white dark:hover:text-secondary" @click="scrollToSection(item.href)">
                            {{ item.name }}
                        </a>
                    </li>
                </ul>
                <button @click="toggleDarkMode" class="text-white ml-20 z-10 hidden md:block">
                    <!-- Show moon icon if darmod off, otherwise show sun icon -->
                    <Icon v-if="!isDarkMode" icon="line-md:moon-filled" class="text-5x1 text-primary"/>
                    <Icon v-else icon="line-md:sunny-outline" class="text-5x1 text-secondary"/>
                </button>
            </nav>
        </div>
    </header>
...
// Reactive repository to track dark mode state
